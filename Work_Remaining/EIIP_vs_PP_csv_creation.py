...
def magtropy_dict(sublevel_dict):
    magtropy_values = [[(sublevel, magtropy(sequence)[0], magtropy(sequence)[1], magnitude_avg(sequence)[0], magnitude_avg(sequence)[1], entropy(sequence)) for sequence in sublevel_dict[sublevel]] for sublevel in sublevel_dict.keys()] #[0] retrieves the value within the list for each sequence
    taxonomic_level = pd.DataFrame((list(itertools.chain.from_iterable(magtropy_values))), columns = ["Sublevel_Name", "EIIP_magtropy", "PP_magtropy", "EIIP_avg_magnitude", "PP_avg_magnitude", "entropy"])
...
saved_path = getcwd() + f"/Paper/num_rep_testing_data2"
