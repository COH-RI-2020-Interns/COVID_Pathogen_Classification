...
# Running Permutation Dictionary through PCC code

# Extract sequence from fasta files
...
# Replace nucleotide bases with numbers using PP representation
...
# Use zero padding to increase the shorter sequence to max length
def normalization(numerical1, numerical2):
    if len(numerical1)>len(numerical2):
        num_zeros = len(numerical1)-len(numerical2)
        for i in range(0, num_zeros):
            numerical2 = np.append(numerical2,0)
    else:
        num_zeros = len(numerical2)-len(numerical1)
        for i in range(0, num_zeros):
            numerical1 = np.append(numerical1, 0)

    return numerical1,numerical2

# DO NOT RUN will not load
# Run the normalized sequences through DFT, find the magnitude, and find PCC
pearsons_dict = {}
